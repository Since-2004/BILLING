...
export default function SaleReturnPage() {
  ...
  const searchInvoice = async () => {
    if (!invoiceNo.trim()) return toast.error('Please enter an invoice number')
    setLoading(true)
    try {
      const res = await fetch(`/api/v1/transactions?search=${encodeURIComponent(invoiceNo)}&type=SALES_INVOICE`)
      ...
      toast.error('Failed to query invoice')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      amount_paid: 0, 
      payment_mode: 'CREDIT',
      notes: `Return against ${originalTx.transaction_no}. Reason: ${reason}`,
      items
    }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-100">Sale Return / Credit Note</h1>
      ...
      <div className="bg-white dark:bg-zinc-900 p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
        ...
        <div className="flex gap-3">
          <input 
            ...
            placeholder="e.g. INV/24-25/00001"
          ...
          />
          <button onClick={searchInvoice} className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition-colors shadow">
            Find Invoice
          </button>
        </div>
      </div>
      ...
      {originalTx && (
        <div className="bg-white dark:bg-zinc-900 p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm space-y-5">
          <div className="flex justify-between items-start">
            <div>
              <p className="font-bold text-zinc-900 dark:text-zinc-100 text-base">Original Invoice: {originalTx.transaction_no}</p>
          ...
          <div className="flex justify-between items-center pt-4 border-t border-zinc-200 dark:border-zinc-800">
            ...
            >
              {loading ? 'Processing...' : 'Process Return & Issue Credit Note'}
            </button>
          </div>
        </div>
      )}
